...
class AnchorTargetCreator(object):
    ...
    def __call__(self, bbox, anchor, img_size):
        # 为每个先验框匹配一个标注框。绝大多数先验框与任何标注框都没交集，这些先验框匹配第一个标注框但标签是背景。即argmax_ious大部分元素为0.
...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label, loc_normalize_mean=(0., 0., 0., 0.), loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        # roi：shape = torch.Size([600, 4])。建议框，网络输出的偏移量转换为像素坐标再经过nms，(xmin,ymin,xmax,ymax)。
        # bbox：shape = (n,4)，n是标注框的数量。type=numpy.ndarray。
        # label：shape = (n,)，n是标签的数量。。type=numpy.ndarray。
        roi = np.concatenate((roi.detach().cpu().numpy(), bbox), axis=0)  # shape=[600+n, 4]。
        # ----------------------------------------------------- #
        #   计算建议框和真实框的重合程度
        # ----------------------------------------------------- #
        iou = bbox_iou(roi, bbox)  # roi.shape=(600+n, 4),bbox.shape=(n, 4),iou.shape=(600+n, n)。为每个建议框找标注框。
        ...
        if len(bbox) == 0:
            ...
            # ---------------------------------------------------------#
            gt_assignment = iou.argmax(axis=1)  # iou.shape=(600+n, n), gt_assignment.shape=(600+n,)。
            # ---------------------------------------------------------#
            #   获得每一个建议框最对应的真实框的iou  [num_roi, ]
            # ---------------------------------------------------------#
            max_iou = iou.max(axis=1)  # iou.shape=(600+n, n), max_iou.shape=(600+n,)。
        ...
        # return:
        # 从建议框中选择与标注框交并比大于等于0.5的建议框作为负样本，负样本最多保留64个。
        # 从建议框中选择与标注框交并比在[0, 0.5)之间的建议框作为正样本，正样本数量等于128-负样本数量。
        # sample_roi：只含正负样本的建议框像素坐标。shape = (128, 4)。
        # gt_roi_loc：正负样本的偏移坐标。shape = (128, 4)。
        # gt_roi_label：正负样本的标签。shape = (128,)。
