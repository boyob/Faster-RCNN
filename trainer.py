...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):  # 图像、标注框、类别标签：numpy.ndarray、list、list，scale=1。
        n = imgs.shape[0]
        img_size = imgs.shape[2:]

        # 获取公用特征层
        base_feature = self.faster_rcnn.extractor(imgs)  # base_feature.shape=[1, 1024, 50, 50]

        # 利用rpn网络获得先验框的得分与调整参数
        rpn_locs, rpn_scores, rois, roi_indices, anchor = \
            self.faster_rcnn.rpn(base_feature, img_size, scale)  # img_size=[800, 800], scale=1.

        rpn_loc_loss_all, rpn_cls_loss_all, roi_loc_loss_all, roi_cls_loss_all = 0, 0, 0, 0
        for i in range(n):
            bbox = bboxes[i]  # shape = (n,4)，n是标注框的数量。
            label = labels[i]  # shape = (n,)，n是标签的数量。
            rpn_loc = rpn_locs[i]  # shape = torch.Size([22500, 4])。网络输出的检测结果，即相对先验框的偏移量(dx,dy,dw,dh)。
            rpn_score = rpn_scores[i]  # shape = torch.Size([22500, 2])。网络输出的分类结果，只区分背景与非背景。
            # shape = torch.Size([600, 4])。建议框，(xmin,ymin,xmax,ymax)。
            # 网络输出的偏移量转换为像素坐标再经过nms获取得分(非背景分类得分)最高的前600个建议框。
            roi = rois[roi_indices == i]
            feature = base_feature[i]  # shape = torch.Size([1024, 50, 50])。主干网络输出的公共特征。
            ...
            # -------------------------------------------------- #
            gt_rpn_loc, gt_rpn_label = self.anchor_target_creator(bbox, anchor, img_size)  # (22500, 4) (22500,)。
            ...
            roi_cls_loc, roi_score = \
                self.faster_rcnn.head(torch.unsqueeze(feature, 0), sample_roi, sample_roi_index, img_size)  # ROIPooling
