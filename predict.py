...
if __name__ == "__main__":
    frcnn = FRCNN()
    mode = "predict"  # predict：表示单张图片预测、video：表示视频检测、fps：表示测试fps。
    # 下面三个参数仅在mode='video'时有效。需要ctrl+c退出才会完成完整的保存步骤，不可直接结束程序。
    video_path = 0  # 可以指定视频路径，为0表示读取摄像头。
    video_save_path = ""  # 保持视频的位置，空路径表示不保存。
